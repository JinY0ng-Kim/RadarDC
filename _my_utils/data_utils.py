...
def load_radar_npy(path, image_shape, data_format='HW'):
    '''
    Loads a radar depth map from a .npy file and maps it to an image-shaped array (no interpolation).

    Arg(s):
        path : str
            path to .npy file (shape: N x 3, columns: x, y, z)
        image_shape : tuple
            (H, W) shape of the target image
        data_format : str
            HW, CHW, HWC
    Returns:
        numpy[float32] : radar depth map
    '''

    radar_points = np.load(path).astype(np.float32)  # shape: (N, 3)
    H, W = image_shape
    ...
    if data_format == 'HW':
        ...
        radar_depth_map = np.expand_dims(radar_depth_map, axis=-1)
    else:
        raise ValueError('Unsupported data format: {}'.format(data_format))

    return radar_depth_map
