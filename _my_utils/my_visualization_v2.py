...
def visualize_results(vmax_depth, cmap, img, lidar_GT, pred, ind, input_radar, semi_depth=None):
    ...
    # --- Visualization ---
    dirpath = './results_l_intrinsic'

    image_save_dir = dirpath + 'image/'
    depth_save_dir = dirpath + 'output/'
    ground_truth_save_dir = dirpath + 'gt/'
    error_save_dir = dirpath + 'error/'
    radar_save_dir = dirpath + 'radar/'
    semi_save_dir = dirpath + 'semi/'
    ...
    # 6. semi_depth 저장
    if semi_depth is not None:
        semi_img = semi_depth.reshape(H, W)
        save_image_exact(semi_img, semi_to_save_name, cmap=color_map, vmin=0.0, vmax=vmax_depth)
