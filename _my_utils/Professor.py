...
def evaluate_all_metric(sample, output, t_valid=0.0001, max_depth=80.0):
    with torch.no_grad():
        # numpy 배열인지 PyTorch 텐서인지 확인
        ...
        if isinstance(sample['gt'], np.ndarray):
            gt = torch.from_numpy(sample['gt']).float()
        else:
            gt = sample['gt'].detach()
        ...
        # Apply both min and max depth thresholds
        mask = torch.logical_and(gt > t_valid, gt < max_depth)
        num_valid = mask.sum()
    ...
    return rmse, mae, irmse, imae, rel, del_1, del_2, del_3
